...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    box_xy = (K.sigmoid(feats[..., :2]) + grid) / \
        K.cast(grid_shape[::-1], K.dtype(feats))  # xy是归一化的值
    box_wh = K.exp(feats[..., 2:4]) * anchors_tensor / \
        K.cast(input_shape[::-1], K.dtype(feats))  # wh是归一化的值
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    Parameters
    ----------
    true_boxes: array, shape=(m, T, 5) m代表批次，T代表最大框数
    ...
    # y_true.shape= [(批次，grid_w,grid_h,3个anchor,box的属性),..,..]

    # Expand dim to apply broadcasting.
    anchors = np.expand_dims(anchors, 0)
    anchor_maxes = anchors / 2.
    anchor_mins = -anchor_maxes
    valid_mask = boxes_wh[..., 0] > 0  # 保留实际存在的box，之前有20个box
    ...
    for b in range(m):
        ...
        if len(wh) == 0:
            continue
        # Expand dim to apply broadcasting.
        wh = np.expand_dims(wh, -2)  # 设shape为（7，2）即有7个box
        box_maxes = wh / 2.
        box_mins = -box_maxes  # shape为(7,1,2)

        intersect_mins = np.maximum(box_mins, anchor_mins)
        intersect_maxes = np.minimum(box_maxes, anchor_maxes)
        intersect_wh = np.maximum(intersect_maxes - intersect_mins, 0.)
        intersect_area = intersect_wh[..., 0] * \
            intersect_wh[..., 1]  # shape为（7,9）
        box_area = wh[..., 0] * wh[..., 1]  # shape为（7,1）
        anchor_area = anchors[..., 0] * anchors[..., 1]  # shape为（1,9）
        iou = intersect_area / (box_area + anchor_area -
                                intersect_area)  # shape为（7,9）
        ...
        for t, n in enumerate(best_anchor):  # t是box的序号，n是最优anchor的序号
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    i = np.floor(true_boxes[b, t, 0] * #x,y在(0,1)之间，再乘以grid
                    ...
                    # 每个box对应的所有anchor尺寸中，最优的anchor才有值
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=True):
    ...
    for l in range(num_layers):  # 每个尺度下
        object_mask = y_true[l][..., 4:5]  # 是否为物体
        true_class_probs = y_true[l][..., 5:]

        grid, raw_pred, pred_xy, pred_wh = \
            yolo_head(yolo_outputs[l], anchors[anchor_mask[l]],
                      num_classes, input_shape, calc_loss=True)  
        # shape(grid)=(13,13,1,2),shape(raw_pred)=(?,13,13,3,6),shape(pred_xy)=(?, 13, 13, 3, 2)
        pred_box = K.concatenate([pred_xy, pred_wh])

        # Darknet raw box to calculate loss.
        # bugfix grid_shapes重复相乘，另一个在preprocess_true_boxes中
        raw_true_xy = y_true[l][..., :2] * \
            grid_shapes[l][::-1] - grid  # 相对网格的位置
        raw_true_wh = K.log(
            y_true[l][..., 2:4] / anchors[anchor_mask[l]] * input_shape[::-1])  # 1
        ...
        #根据IoU忽略阈值生成ignore_mask,shape(ignore_mask)=(batch, grid_shape[0],  grid_shape[1], 3, 1)
        ...
        confidence_loss = object_mask * K.binary_crossentropy(object_mask, raw_pred[..., 4:5], from_logits=True) + \
            ...
            # 第一部分是物体损失。第二部分是非物体的损失，并忽略IOU大于阈值的框所造成的损失
